...
function Profile() {
  ...
  async function saveProfile() {
    const { error } = await supabase
      .from("profiles")
      .upsert({ id: user!.id, full_name: name, phone });
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="space-y-3 p-5">
          ...
          {addresses.map((a) => (
            <div key={a.id} className="flex items-start gap-3 rounded-xl border p-3">
              ...
              <p className="flex-1 text-sm">
                {a.formatted_address}
                {a.is_default && <span className="ml-2 text-xs text-success">Default</span>}
              </p>
              <Button variant="ghost" size="icon" onClick={() => delAddr(a.id)}>
          ...
          <div className="flex gap-2">
            ...
            />
            <Button size="icon" onClick={addAddress}>
      ...
      <Card>
        <CardContent className="space-y-3 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Payment methods are stubbed in this preview. Real card & EFT support will be added
            later.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="space-y-3 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Chat with the Zap Gas team. Messages auto-clear after 14 days.
          </p>
          <Link to="/app/support">
      ...
      <Card className="border-accent/40">
        <CardContent className="space-y-3 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            After granting, open <span className="font-mono">/admin</span> or{" "}
            <span className="font-mono">/driver</span> in the URL bar.
          </p>
        </CardContent>
      </Card>
    </div>
